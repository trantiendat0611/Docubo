...
| 29 | **Bẫy nghiêm trọng hơn cả #28.** Chạy `--judge` chốt số lần cuối trên 26 câu gốc thì `hit@8` rớt từ 1.000 xuống **0.667**, `false_refusal_rate` từ 0.000 vọt lên **0.824** — 82% câu đáng trả lời bị từ chối oan. Không phải bản sửa bẫy #28 gây ra: đo trực tiếp retrieval bằng service_role thì `testtv1.pdf` vẫn cho cosine 0.79 — dữ liệu hoàn toàn khoẻ | Gọi thẳng `/api/chat` bằng token thật (không phải service_role) thì tái hiện y hệt: câu hỏi về `testtv1.pdf` bị từ chối cấu trúc. Tra `documents.owner_id` thì lộ ra: **corpus khai báo 3 tài liệu đang thuộc VỀ HAI TÀI KHOẢN KHÁC NHAU** — `testtv1.pdf`/`testta1.pdf`/bản gốc `2402.00253v2.pdf` thuộc `trantiendat.cl@gmail.com` (tạo 10/08, ngày đầu dự án), còn mọi tài liệu từ 12/08 trở đi (`NET Roadmap`, `2608.18058v1.pdf`, ảnh dán, và một bản `2402.00253v2.pdf` thứ hai) thuộc `11a2trantiendat@gmail.com` — token dùng hôm nay. RLS (`chunks_select_own using (owner_id = auth.uid())`) chặn **đúng theo thiết kế**: đây không phải lỗi bảo mật, mà là hai tài khoản test đang sở hữu hai nửa không giao nhau của cùng một corpus. Đối chiếu ngược: `c-001` bị chấm "answer" nhưng đọc nguyên văn thì là từ chối văn xuôi — *"there is no mention of..."* — vì nó không thấy được `testta1.pdf`, chỉ thấy hai tài liệu nó có quyền. Việc này cũng giải thích lại bẫy #26/27 ở một tầng sâu hơn: `2402.00253v2.pdf` có **hai bản dưới hai chủ sở hữu** không phải chỉ vì vision không tất định (nguyên nhân kĩ thuật trực tiếp), mà vì người dùng đổi tài khoản test giữa chừng và tải lại file đã có, không biết bản cũ đang vô hình dưới tài khoản kia | **Chưa sửa — cần quyết định của người dùng trước khi tốn quota.** Cách đúng: nạp lại `testtv1.pdf` + `testta1.pdf` dưới tài khoản hiện hành (`11a2trantiendat@gmail.com`), để một tài khoản sở hữu trọn corpus khai báo — đúng với cách một người dùng thật sẽ trải nghiệm app. Rẻ hơn tưởng: batch 8 trang/request nên chỉ tốn **~10 request vision** (5+68 trang), không phải 73. Báo cáo `--judge` hôm nay (`eval-full-20260824-071506.json`) **giữ lại làm bằng chứng cho chính phát hiện này**, nhưng `faithfulness = 0.986` của nó **không được dùng làm số chốt cho `SKILL` §0** — nó đo trên một corpus bị thu hẹp bất thường do lỗi vận hành, không phải chất lượng hệ thống thật. Phải chạy lại `--judge` sau khi sửa quyền sở hữu. Bài học: **một token hợp lệ không đồng nghĩa nó nhìn thấy đúng corpus đang được khai báo** — với hệ thống cô lập theo tài khoản, "chạy lại đúng lệnh cũ" không đủ để tái lập một phép đo nếu danh tính người chạy đổi giữa hai lần |
| 30 | **Sửa một bên, quên bên song song — đúng bài học đã ghi ở bẫy #28 nhưng tự mắc lại.** Chạy `--judge` chốt số sau khi sửa bẫy #28 thì `faithfulness` tụt xuống **0.835** — thấp hơn ngưỡng 0.90, và thấp hơn hẳn mọi lần đo trước (luôn 1.000). Soi từng câu điểm thấp: `g-001`/`g-002` — hai câu `figure` duy nhất trong bộ — đều bị chấm **0.0**, dù câu trả lời khớp gần như nguyên văn `expected_points` trong dataset (*"Kilo 10^3, Mega 10^6..."*, *"màu sắc, số nhân, số đuôi"*). Model không bịa — bộ đo bịa | Bẫy #28 sửa `buildContext()` bên TypeScript (`src/lib/prompt.ts`) — nơi model **sinh** câu trả lời. Nhưng `eval/judge.py` có một `build_context()` **Python riêng**, viết từ trước, đọc thẳng `display_text` từ database để dựng lại đúng context model đã thấy (tránh gọi lại retrieval — bẫy #17 đã ghi lý do). `display_text` trong database **chưa từng được thay đổi** bởi bẫy #28 — bản sửa đó chỉ tác động lúc dựng prompt, không tác động dữ liệu đã lưu — nên hàm Python này vẫn đọc nguyên `[[FIGURE:id]]` chưa thay thế. Model trả lời đúng dựa trên context đã resolve (phía sinh câu trả lời); giám khảo chấm dựa trên context chưa resolve (phía Python) — hai bên nhìn hai context khác nhau cho cùng một câu hỏi | Thêm `_resolve_figures()` vào `judge.py`, cổng lại đúng logic `resolveFigures()` bên TS (không phải `toEmbedText`/`_to_embed_text` — hai hàm đó dùng cho tìm kiếm, ghép chuỗi khác định dạng). Thêm cột `figure_refs` vào `chunks_by_id()` (`ingest/pipeline/store.py`) vì trước đó hàm này còn chưa từng chọn cột đó ra. Xác minh bằng dữ liệu thật, không tốn quota: đọc thẳng 3 chunk có `has_figure=True` từ database, gọi `_resolve_figures()`, thấy bảng dữ liệu tế bào (`color | #nuclei | #tails`) hiện ra đúng thay vì placeholder. 2 test mới. **Bài học kép:** (a) một hàm ghi rõ trong docstring *"Same block shape as buildContext in src/lib/prompt.ts"* là một lời hứa đồng bộ **không có gì ép buộc** — không type nào, không test nào từng kiểm tra hai bên có thực sự giống nhau; (b) sửa một bug về "hai bản triển khai song song lệch nhau" (bẫy #28) mà không rà xem còn BAO NHIÊU bản sao khác của cùng logic đang tồn tại là tự để lại đúng bẫy đó ở một nơi khác — thứ vừa học được 24 giờ trước |
